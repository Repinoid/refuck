...
type Iteration14Suite struct {
	...
	serverPort    string
	serverProcess *BackgroundProcess
...
func (suite *Iteration14Suite) serverUp(ctx context.Context, envs, args []string, port string) {
	suite.serverProcess = NewBackgroundProcess(context.Background(), flagServerBinaryPath,
		WithEnv(envs...),
		WithArgs(args...),
	)
...
func (suite *Iteration14Suite) agentUp(ctx context.Context, envs, args []string, port string) {
	suite.agentProcess = NewBackgroundProcess(context.Background(), flagAgentBinaryPath,
		WithEnv(envs...),
		WithArgs(args...),
	)
